...
while i < len(lines):
    ...
    if re.search(r'public function (?!__construct)', line):
        ...
        new_lines.append(f"        // PROTEKSI_RIZX_NEST: Hanya admin ID 1")
        new_lines.append(f"        if (!Auth::user() || (int) Auth::user()->id !== 1) {{")
        new_lines.append(f"            abort(403, '\\u{1F6AB} Akses ditolak! Hanya admin ID 1 yang dapat mengakses menu Nests. \\u00A9{brand_text}');")
        new_lines.append(f"        }}")
